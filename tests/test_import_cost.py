...
@pytest.mark.parametrize("confidence", [0.5, 0.8, 0.9, 0.95, 0.975, 0.99, 0.995, 0.999])
@pytest.mark.parametrize(
...
def test_runs_needed_returns_the_same_integer(p: float, min_rate: float, confidence: float) -> None:
    ...
    while high <= 10_000_000:
        if _wilson(min(round(p * high), high), high, z_scipy)[0] >= min_rate:
            break
        low, high = high + 1, high * 2
    while low < high:
        mid = (low + high) // 2
        if _wilson(min(round(p * mid), mid), mid, z_scipy)[0] >= min_rate:
            high = mid
        else:
            low = mid + 1
    assert answer == low
