...
def _one_sided_z(confidence: float) -> float:
    """One-sided z for a confidence level, from the stdlib.

    Deliberately not routed through :func:`wilson_lower_bound`: the power search
    below evaluates the bound tens of thousands of times, and paying for argument
    validation and a fresh inverse-normal on each of them turned a failing
...
def _gate_power(p: float, min_rate: float, n: int, confidence: float) -> float:
    ...
    """
    return _binomial_tail(_minimum_successes(min_rate, n, _one_sided_z(confidence)), n, p)
